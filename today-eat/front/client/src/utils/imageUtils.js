 ...
 * 将图片裁剪为 1:1 正方形并压缩
 */
export async function cropAndCompressImage(file, crop) {
    // 创建 Image 对象
    ...
    // 绘制裁剪后的图片
    ctx.drawImage(image, sourceX, sourceY, sourceSizeX, sourceSizeY, 0, 0, TARGET_SIZE, TARGET_SIZE);
 ...
 * 压缩 Canvas 图片至指定大小
 */
async function compressCanvas(canvas, mimeType) {
    let quality = 0.9;
    let blob;
    do {
